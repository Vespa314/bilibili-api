...
if __name__ == "__main__":
    ...
    for bangumi in bangumilist:
        ...
        for vedio in vediolist:
            ...
            filepath = "%s%s.txt"%(path,vedio.episode);
            
            if not os.path.isfile(filepath):
                fid = open(filepath,"a+");
                bgminfo = "Name:%s\nav=%s\nfrom:%s\nupdate day:%s\n"%(vedioinfo.title,vedioinfo.aid,vedio.src,vedioinfo.date);
                fid.write(bgminfo);
            else:
                fid = open(filepath,"a+");
            cur_time = time.strftime('%Y-%m-%d %H:%M',time.localtime(time.time()))
            
            info = "%s: gk(%s) pl(%s) dm(%s) sc(%s) yb(%s)\n"%(cur_time,vedioinfo.guankan,vedioinfo.commentNumber,vedioinfo.danmu,vedioinfo.shoucang,vedioinfo.coin)
            fid.write(info)
            fid.close();
            time.sleep(1)
